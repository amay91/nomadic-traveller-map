...
**Status:** **Done and in active use.** `app/` is the real, running app. 34/34 automated tests pass. Size: 78.8 KB of an 80 KB code-excluding-comments budget (§4.1) — **1.2 KB of headroom left, the tightest this budget has ever been.** The next feature that touches `app/` should run `tools/measure-size.mjs` before writing much code, not after — there is very little room left to discover a problem late. **Live at https://amay91.github.io/nomadic-traveller-map/ since 2026-09-15** (`plan.md` §6, `tasks.md` Phase 3). Phase 3 is complete apart from D5, an optional custom domain.
...
| `app.js` | 79.0 KB | 41.7 KB | 37.3 KB |
| **Total** | **149.1 KB** | **70.3 KB** | **78.8 KB of 80 KB — 1.2 KB headroom** |
